...
- **ConversationModels** owns animated NPC and player copies during gossip, quest, trainer, and merchant interactions. It reads native unit appearances without any Voiceover dependency. Visible native PlayerModel frames own unit appearances, lighting, facing, and animations. Each has an alpha-zero ModelScene child that loads only the visible model's file ID for bounds measurement; helper actors have no script callbacks and never render the displayed copies. The existing update loop polls helper loading and valid bounds. The combined minimum and maximum vertical bounds determine one shared camera height and target. Clearance from that height and the horizontal bounds corners rotated to each model’s default opening facing estimate one shared camera distance using a nominal field of view and the shared edge margin. PlayerModel does not expose its projection field of view; equipment effects and animation excursions are outside this estimate. Missing bounds retain the established camera distance without hiding visible models. NPC rotation keeps the shared camera fit fixed; some edge clipping is allowed at other angles. Camera writes require a loaded visible model and confirmed custom camera. Model changes invalidate measurements, and closing clears both copies and helpers; the NPC stays left and the player stays right, behind menu controls at the preserved backdrop's frame level. The module retains models across brief page transitions and inventory pane switches, refreshes changed appearances, and clears them on interaction closure. Per-frame mapped camera-stick input rotates only the visible NPC copy, retaining its facing across page switches and resetting on closure or NPC changes. Native interaction events and visible menu changes trigger occasional contextual model emotes, with per-model cooldowns and completion/timeout restoration to standing.
