...
- **Conversation models:** Gossip, quest, trainer, and merchant interactions display the NPC on the left and your character on the right, in fixed screen-relative positions behind menus. Merchant/inventory switching preserves their placement. Both copies use the same model scale and shared camera distance, preserving relative sizes. Native PlayerModel frames retain unit appearances, lighting, and facing. Invisible helpers measure matching base-model files to estimate a shared camera distance from their bounds and viewport dimensions. Smaller pairs zoom closer with no menu enlargement cap. PlayerModel does not expose its field of view, and base-model bounds may omit equipment effects, so exact visible fitting requires in-game verification. Models use current unit appearances, with a 35% chance per model of a contextual greeting, conversation gesture, or quest acknowledgment when menus open or change. A three-second cooldown limits repeated gestures, and models return to standing afterward; exact world movement or emote synchronization is unavailable. Move the right stick (R3) left or right to rotate the NPC model; its angle persists across conversation pages and resets when the interaction closes or the NPC changes. Models do not intercept input and hide after the interaction closes. This feature works independently of Voiceover.
